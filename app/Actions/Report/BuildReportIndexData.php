...
class BuildReportIndexData
{
    public function __invoke(): array
    {
        ...
        $currentWeek = $this->buildWeekDataset(
            ...
            tz: $tz,
            isCurrentWeek: true,
            pendingContext: 'this week'
        );

        $upcomingWeek = $this->buildWeekDataset(
        ...
        })->values()->map(function (Report $item) use ($now, $tz, $totalKaryawan, $currentUsername) {
            $summary = $this->summarizeWeekCompletion($item, $totalKaryawan, $tz);

            return $this->transformPendingReport(
                report: $item,
                now: $now,
                tz: $tz,
                summary: $summary,
        ...
        })->values()->map(function (Report $item) use ($tz) {
            return [
                'code' => $item->code,
                'exported_at_label' => $item->exported_at
                    ? $item->exported_at->timezone($tz)->format('Y-m-d H:i')
                    : '—',
                'exported_by_label' => $item->exporter?->name
                    ?? $item->exported_by
                    ?? '—',
            ];
        })->values();
    ...
    protected function buildWeekDataset(
        ...
        [$windowStatusLabel, $windowBadgeClass, $windowHelp] = $this->resolveWindowStatus(
            ...
            deadline: $deadline,
            tz: $tz,
            hasPastExport: $hasPastExport,
            futureExportAt: $hasFutureExport ? $report->exported_at : null
        );
        ...
        $pendingWarning = count($pendingUsers) > 0
            ? sprintf(
                '%d of %d karyawan still have incomplete selections for %s.',
                count($pendingUsers),
                max($totalKaryawan, 0),
                $pendingContext
            )
    ...
    protected function transformPendingReport(
        ...
        $pendingMonday = Project::mondayFromMonthWeekCode($report->code, $tz);
        $availableAt = $pendingMonday->copy()->subDays(3)->startOfDay();
        $canExport = $now->greaterThanOrEqualTo($availableAt);

        $pendingCompleted = $summary['completed'];
        $pendingIncomplete = $summary['pending'];
        $pendingTotalUsers = $pendingCompleted + $pendingIncomplete;

        $exportedByCurrent = $currentUsername
            && $report->exported_by
            && $report->exported_by === $currentUsername;

        if ($exportedByCurrent) {
            if ($report->exported_at) {
                $statusLabel = 'EXPORTED';
                $statusBadgeClass = 'bg-success';
                $statusNote = 'Export scheduled by you for ' . $report->exported_at->timezone($tz)->format('D, d M Y H:i') . '.';
                ...
                $statusLabel = 'EXPORTED';
                $statusBadgeClass = 'bg-success';
                $statusNote = 'You exported this report earlier in this session.';
                $repeatMessage = 'This report was already exported earlier in this session.';
            }
        } elseif ($summary['ready']) {
            $statusLabel = 'READY';
            $statusBadgeClass = 'bg-info';
            $statusNote = 'All karyawan have completed their selections.';
            $repeatMessage = null;
        } else {
            $statusLabel = 'PENDING';
        ...
        $pendingWarning = ($pendingIncomplete > 0 && $pendingTotalUsers > 0)
            ? sprintf('%d of %d karyawan still have incomplete selections for this week.', $pendingIncomplete, $pendingTotalUsers)
            : '';

        return [
            'code' => $report->code,
            'created_at_label' => $report->created_at
                ? $report->created_at->timezone($tz)->format('Y-m-d')
                : '—',
            'status_label' => $statusLabel,
            'status_badge_class' => $statusBadgeClass,
            'status_note' => $statusNote,
            'export_route' => route('masterReport.export', ['masterReport' => $report->code]),
            'available_label' => $availableAt->format('D, d M Y H:i'),
            'pending_warning' => $pendingWarning,
            'repeat_warning' => $repeatMessage,
            'can_export' => $canExport,
            'incomplete_summary' => (!$summary['ready'] && $pendingIncomplete > 0)
                ? sprintf('Incomplete: %d karyawan.', $pendingIncomplete)
                : null,
        ];
    }
}
